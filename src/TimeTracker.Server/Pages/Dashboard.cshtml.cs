...
public class DashboardModel : PageModel
{
    ...
    public TimeBreakdown Time { get; private set; } = new(0, 0, 0, 0);
    ...
    public double? LockedChangePercent { get; private set; }
    ...
    public async Task OnGetAsync(CancellationToken cancellationToken)
    {
        ...
        if (fromUtc is { } from && toUtcExclusive is { } to)
        {
            ...
            ActiveChangePercent = PercentChange(previous.Time.ActiveSeconds, current.Time.ActiveSeconds);
            IdleChangePercent = PercentChange(previous.Time.IdleSeconds, current.Time.IdleSeconds);
            LockedChangePercent = PercentChange(previous.Time.LockedSeconds, current.Time.LockedSeconds);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var time = await LoadTimeBreakdownAsync(excludedUserNames, fromUtc, toUtcExclusive, cancellationToken);
